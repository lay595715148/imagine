...
Mongo.prototype.select = function select(selector, fields, opts, fn) {
    var assert = require('assert');
    var me = this;
    var link = this.link;
    var table = this.model.table();
...
Mongo.prototype.selectOne = function selectOne(selector, fields, opts, fn) {
    ...
    me.connect(function() {
        $.Log.info('mongo select one', table, $.Util.toString(selector), $.Util.toString(fields));
        link.collection(table).find(selector, fields, opts).toArray(function(err, result) {
            assert.equal(null, err);
            me.result = result;
            var item = null, obj = {};
...
Mongo.prototype.get = function get(id, opts, fn) {
    $.Log.debug(JSON.stringify($.get('pic_size')));
};
