...
Action.prototype.isLogin = function isLogin() {
    ...
    var info = $.Util.toJson($.util.Encryption.decrypt(key));
    return $.Util.isNUmber(info[1]) && info[1] > 0 ? true : false;
};
Action.prototype.setLogined = function setLogined(userid) {
    ...
    var info = [confusion, userid, $.Util.time(), 0];//
    var code = cookie[sessid] = $.util.Encryption.encrypt(JSON.stringify(info));//$.util.Base64.encode()
