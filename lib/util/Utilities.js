...
(function() {
    ...
    Utilities.classname = 'util.Utilities';
    ...
    Utilities.calledStack = function(regex) {
        var stackRegex = Utilities.isRegExp(regex) ? regex : new RegExp("Error.+?\t.+?\t.+?\t +?at (.+?)\t");
    ...
    Utilities.extractCalledFromStack = function(regex) {
        var stackRegex = Utilities.isRegExp(regex) ? regex : new RegExp("Error.+?\t.+?\t.+?\t +?at (.+?)\t");
        var stack = new Error().stack;
        stack = stack.replace(/\n/g, "\t");
        var r = stackRegex.exec(stack);
    ...
    Utilities.getClassname = function(obj) {
        if(Utilities.isObject(obj) && !Utilities.isNull(obj)) {
            return obj.constructor.classname ? obj.constructor.classname : (obj.constructor.name ? obj.constructor.name : 'Object');
        } else if(Utilities.isFunction(obj)) {
            return obj.classname ? obj.classname : obj.name;
        } else {
            return false;
        }
    };
    ...
    Utilities.isA = function(o, O) {
        return o instanceof O ? true : false;
    };
    /**
     * @api public
     */
     ...
     * @api public
     */
    Utilities.isException = function(o) {
        if(Utilities.isError(o)) {
            return true;
        } else if(Utilities.isA(o, $.core.Exception)) {
            return true;
        } else {
            return false;
        }
