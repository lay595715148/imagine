...
OAuth2Service.classname = 'service.OAuth2Service';
...
OAuth2Service.prototype.checkRequest = function(scope, request_type) {
    var ret = true;
    var get = scope.get(), post = scope.post(), user = scope.user(), request = scope.request();
    post = scope.request();//
    ...
    switch(request_type) {
        ...
        case OAuth2Service.REQUEST_TYPE_SHOW:
            if($.Util.isUndefined(request.access_token) || $.Util.isUndefined(request.userid)) {
                ret = false;
            }
            break;
        default:
