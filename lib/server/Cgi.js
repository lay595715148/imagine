...
(function() {
    ...
    Cgi.prototype.open = function open() {
        ...
        app.use(function(req, res, next) {
            ...
            if(_path === '/test') {
                $.Log.info($.htmlout('<span style="color: #f00;">------------ TST -------------</span>'));
                $.App.test(req, res, api_url, function() {
                    $.Log.info('------------ TST* ------------');
                });
            } else if(_path === '/'){
                $.Log.info($.htmlout('<span style="color: #0cc;">------------ JSN -------------</span>'));
                $.App.json(req, res, function() {
                    $.Log.info('------------ JSN* ------------');
                });
            } else if(/^\/io\/.*/.test(_path)){
                $.Log.info($.htmlout('<span style="color: #0ff;">------------ IO --------------</span>'));
                $.App.io(io, req, res, function() {
                    $.Log.info('------------ IO* -------------');
                });
            } else {
                $.Log.info($.htmlout('<span style="color: #0f0;">------------ API -------------</span>'));
